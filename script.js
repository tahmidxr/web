document.addEventListener('DOMContentLoaded', () => {
    ...
    // NEW: Audio Context and Sound for button clicks
    let audioContext;
    ...
    // Initialize audio on first user interaction or document load
    initAudio();

    // Attach click sound to relevant interactive elements
    const clickableElements = [
        heroWorkButton, // "Work With Me" button
        ...document.querySelectorAll('.nav-links a'), // Nav menu links
        hamburgerMenu, // Hamburger menu icon
        ...document.querySelectorAll('.works-grid img'), // Recent works images (acting as interactive elements)
        ...document.querySelectorAll('.social-link'), // Social media icons
        sendButton // Contact form send button
    ].filter(Boolean); // Filter out any null/undefined elements

    clickableElements.forEach(element => {
        element.addEventListener('click', playSound);
        // For touch devices, 'touchend' might be more reliable after a tap
        element.addEventListener('touchend', (e) => {
            // Prevent double-playing if both click and touchend fire
            e.preventDefault(); 
            playSound();
        });
    });
