document.addEventListener('DOMContentLoaded', () => {
    ...
    const scrollingHeadlineTextCollab = document.querySelector('.scrolling-headline-text:not(.skills-headline-text)');
    const phraseCollab = "Collaborations & Platforms";
    const repetitionCountCollab = 5; // Repeat the phrase multiple times to ensure it covers the screen

    // Use specific variables for collaborations
    let collabLastScrollY = window.scrollY;
    let collabCurrentTranslateX = 0;
    const scrollSpeedFactor = 0.5; // Controls how fast the text moves relative to scroll

    if (scrollingHeadlineTextCollab) {
        // Dynamically add repeated phrases
        for (let i = 0; i < repetitionCountCollab; i++) {
            const span = document.createElement('span');
            span.textContent = phraseCollab;
            span.classList.add('scrolling-text-phrase'); // Add new class
            scrollingHeadlineTextCollab.appendChild(span);
        }

        function animateScrollingHeadlineCollab() {
            const deltaScroll = window.scrollY - collabLastScrollY;
            ...
            collabCurrentTranslateX -= deltaScroll * scrollSpeedFactor;

            scrollingHeadlineTextCollab.style.transform = `translateX(${collabCurrentTranslateX}px)`;
            collabLastScrollY = window.scrollY; // Update lastScrollY for the next frame

            requestAnimationFrame(animateScrollingHeadlineCollab);
        }
    ...
    // NEW: Dynamic Scrolling Headline for Skills Section
    const scrollingHeadlineTextSkills = document.querySelector('.skills-headline-text');
    const phraseSkills = "Skills & Expertise";
    const repetitionCountSkills = 5;

    // Use specific variables for skills
    let skillsLastScrollY = window.scrollY;
    let skillsCurrentTranslateX = 0;

    if (scrollingHeadlineTextSkills) {
        // Dynamically add repeated phrases
        for (let i = 0; i < repetitionCountSkills; i++) {
            const span = document.createElement('span');
            span.textContent = phraseSkills;
            span.classList.add('scrolling-text-phrase'); // Add new class
            scrollingHeadlineTextSkills.appendChild(span);
        }

        function animateScrollingHeadlineSkills() {
            const deltaScroll = window.scrollY - skillsLastScrollY;
            skillsCurrentTranslateX -= deltaScroll * scrollSpeedFactor;
            scrollingHeadlineTextSkills.style.transform = `translateX(${skillsCurrentTranslateX}px)`;
            skillsLastScrollY = window.scrollY;
            requestAnimationFrame(animateScrollingHeadlineSkills);
        }
        requestAnimationFrame(animateScrollingHeadlineSkills);
    }
